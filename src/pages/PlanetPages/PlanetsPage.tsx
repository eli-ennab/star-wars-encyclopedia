...
const PlanetsPage = () => {
	...
	const getPlanets = async (_endpoint: string) => {
		setError(null)
		setLoading(true)
		setResult(null)
		setSearchInput("")
		...
		try {
			const data = await getResourcesByPage<SW_PlanetsResponse|null>('/planets', Number(paramPage))
			setResult(data)	
		} catch (err: any) {
	...
	const searchSWPlanets = async (searchQuery: string) => {
		setError(null)
		setLoading(true)
		setResult(null)

		try {
			const data = await searchPlanets(searchQuery, Number(paramPage))
			setResult(data)
		} catch (err: any) {
			setError(err.message)
		}
